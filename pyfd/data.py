...
def cache_data(dataset, filename):
    """ Load the data from an URL and cache it locally"""
    ...
    if not os.path.exists(filepath):
        ...
        urlretrieve(GITHUB_DATA_URL + dataset + "/" + filename, filepath)
    return filepath
...
def get_data_bike():
    ...
    df = pd.read_csv( cache_data("Bike-Sharing", "hour.csv") )
...
def get_data_adults(use_target_encoder=False, remove_gender=False):
    ...
    # load test
    raw_data_2 = np.genfromtxt( cache_data('Adult-Income','adult.test'), delimiter=', ', dtype=str, skip_header=1 )
...
def get_data_marketing(use_target_encoder=False):
    ...
    # load train
    df = pd.read_csv( cache_data('marketing', 'marketing.csv'), delimiter=";")
    feature_names = df.columns[:-1]
...
def get_data_credit(use_target_encoder=False):
    ...
    # load train
    df = pd.read_csv( cache_data('default_credit', 'default_credit.csv') )
...
def get_data_kin8nm():
    ...
    # load train
    df = pd.read_csv( cache_data('kin8nm', 'dataset_2175_kin8nm.csv'), delimiter="," )
    feature_names = list(df.columns[:-1])
    outcome = df.columns[-1]
...
def get_data_kaggle_housing(remove_correlations=False, submission=False):
    ...
    if submission:
        df = pd.read_csv( cache_data('kaggle_houses', 'test.csv'), delimiter="," )
    else:
        df = pd.read_csv( cache_data('kaggle_houses', 'train.csv'), delimiter="," )
    Id = df["Id"]
