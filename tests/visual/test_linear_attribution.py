...
for task in ["regression", "classification"]:
    print(f"Doing {task}")
    X, y = generate_data(5000, noise, task=task)
    model = model_type[task].fit(X, y)

    line = np.linspace(-3, 3, 10)
    foreground = np.column_stack((line, line))
    decomposition_1 = get_components_linear(model, foreground, X)
    if task == "classification":
        decomposition_2 = get_components_brute_force(model.decision_function, foreground, X)
    else:
        decomposition_2 = get_components_brute_force(model.predict, foreground, X)
